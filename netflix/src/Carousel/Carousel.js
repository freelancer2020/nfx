...
const ArrowLeftGo = styled.div`
    ...
    transform: ${props => props.act ? 'rotate(-315deg)': 'rotate(45deg)'};
    transition: 0.5s ease-out;
    cursor: pointer;
    z-index: 6;
`;

const ArrowRightGo = styled.div`
    width: 20px;
    height: 20px;
    position: absolute;
    right: 0;
...
const Carousel = () => {
  ...
  const [ active, setactive ] = useState({act: false, act2: false});
  ...
  const animRight2 = () => {
    setactive({act2: false});
  }

    const controlFlowLeft = () => {
        
        const teatre = document.getElementsByClassName('teatre')[0];
        const dataView = document.getElementsByClassName('teatre')[0].children;

        new Promise(resolve => {
            const lastElement = dataView[dataView.length -1];
            const elementCopy = dataView[dataView.length -1];
            elementCopy.style.marginLeft = '-16vw';
            dataView[dataView.length -1].remove();
            resolve(lastElement);
          })
          .then(newElement => {
           teatre.insertAdjacentElement('afterbegin', newElement);
          })
          .then(() => {
            setTimeout(() => {
              dataView[0].style.marginLeft = '5px';
            }, 50);
          })
          .catch(err => alert(err.message));
    }

    const controlFlowRight = () => {
      //alert("D")
        ...
        /*
        new Promise(resolve => {
          const lastElement = dataView[0];
          const elementCopy = dataView[0];
          dataView[0].style.marginLeft = '-16vw';
         // elementCopy.remove();
          resolve(lastElement);
        })
        .then(newElement => {
         teatre.insertAdjacentElement('beforeend', newElement);
        })
        .then(() => {
          setTimeout(() => {
            dataView[0].style.marginLeft = '5px';
        ...
        */
        ...
        dataView[0].style.marginLeft = '-16vw';
        let coEl = dataView[0].cloneNode(true);
        setTimeout(() => {
          dataView[0].remove();
          teatre.insertAdjacentElement('beforeend', coEl);
        }, 100);
    ...
    const UI = (
        <div className = 'carousel-container' >
            ...
            <ArrowLeftGo act = {active.act} onClick = { controlFlowLeft } onMouseOver = { anim } onMouseOut = { anim2 }></ArrowLeftGo>
            

            <div className = 'teatre'>
            ...
            <ArrowRightGo act = {active.act2} onClick = { controlFlowRight } onMouseOver = { animRight } onMouseOut = { animRight2 }></ArrowRightGo>   
